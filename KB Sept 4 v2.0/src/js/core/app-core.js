 ...
 * APP CORE
 * ==================
 * UPDATED: Removed the reference to the deprecated 'debugLog' element
 * from the DOM cache to fix the "DOM Validation Failed" error at startup.
 *
 * This file defines the application's shared state, DOM element cache,
 * and core event handler functions.
 */
...
const state = {
    ...
    activeModal: null,
    activePopover: null,
    searchHistory: [], // Initialized from storage in app-main.js
    settings: {},
    searchService: null, // Initialized in app-main.js
};
...
const dom = {
    ...
    toolsKbSearch: null, // Queried for when template is loaded
    kbList: null, // Queried for when template is loaded
    ...
    toggleInPageConsole: document.getElementById('toggleInPageConsole'),
...
/**
 * DOM VALIDATOR
 * This function loops through the `dom` object and ensures that every
 * element was successfully found. If any element is null (due to a typo
 * in the ID), it will throw a specific error, making debugging instant.
 */
function validateDom() {
    for (const key in dom) {
        // We only validate elements that are expected to be in the initial HTML.
        // Elements loaded from a <template> are intentionally null at startup.
        if (dom[key] === null && key !== 'toolsKbSearch' && key !== 'kbList') {
            throw new Error(`DOM Validation Failed: The element with ID '${key}' was not found in index.html. Please check for typos.`);
        }
    }
...
function handleSearchInput() {
    ...
    if (state.searchService) {
        const results = state.searchService.search(query);
